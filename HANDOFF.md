...
| `sync-files-from-sharepoint` | Project page load (auto-sync) + admin button + bulk-sync script | Pulls files from `Purchase Order/` and `Quote/` subfolders + root `Certificate*.pdf` files for a given project. Equipment certificates come via the loadout pipeline: Load Out List (JobNumber) → shipped Assembly SNs → Hydro-Wates Inventory (FolderPath) → the equipment's Asset Records folder — syncing ONLY the cert in force at that unit's load-out: the newest cert dated on/before the unit's Load Out List date (explicit load-out/ship date column when the list has one, else the row's Created time; 3-day grace), so old jobs keep the historically-correct cert rather than later recalibrations. Cert dates parse from the MM-DD-YY filename suffix (SharePoint created date as fallback). Certs outside that rule previously synced into the project are deleted on the next sync. Dedupes by SharePoint item ID. Supports `certsOnly: true` body param to skip subfolders + equipment. |
