...
| Customer's Zoho data is stale | Zoho daily API quota exhausted (error code 45, "exceeded the maximum call rate limit of 10,000" — waits out the overnight reset), or Zoho refresh token revoked. Admin page loads auto-sync, so persistent staleness means the function itself is failing — the dashboard shows the failure reason under "Last synced". | `sync-zoho` function logs | Quota: wait for reset (or find what's burning calls). Token: reauth Zoho self-client; rotate `ZOHO_REFRESH_TOKEN` secret |
...
| `SHAREPOINT_PO_ROOT_FOLDER` | Path under Shared Documents for Hydro-Wates POs (default `Hydro-Wates/Commercial Proposals`) | Edit in dashboard if folder layout changes | n/a |
| `SHAREPOINT_SENTINEL_ROOT_FOLDER` | Path under Shared Documents for Sentinel Instruments POs (default `Sentinel Instruments/Commercial Proposals`; used for Q-coded jobs) | Edit in dashboard if folder layout changes | n/a |
...
|---|---|---|
| `sync-zoho` | Auto-fired on every page load in admin sessions (`SyncContext`, in-flight lock + 2-min cooldown). NOTE: the "hourly cron" older docs mention does not exist — pg_cron is not installed on the LifeOSBase database (verified 2026-07-28). | Pulls customers/projects/invoices from Zoho Books. Delta sync: per-contact detail + contact-person calls only for contacts modified since the stored watermark (`cportal_app_settings.zoho_sync_watermark`); full sweep at most daily (`zoho_full_synced_at`) or on `{ "full": true }`. Keeps runs ~10-20 Zoho calls vs ~2-per-contact — the org's Zoho budget is 10k calls/day (hit on 2026-07-28, which blocks all syncing until the overnight reset). Auto-adds project members on email match. Sends Graph email on status changes. |
| `sync-files-from-sharepoint` | Project page load (auto-sync) + admin button + bulk-sync script | Pulls files from `Purchase Order/` and `Quote/` subfolders + root `Certificate*.pdf` files for a given project. Equipment certificates come via the loadout pipeline: Load Out List (JobNumber) → shipped Assembly SNs → Hydro-Wates Inventory (FolderPath) → the equipment's Asset Records folder (root + one level of "cert"-named subfolders, e.g. `Calibration Certificates/`) — syncing ONLY the cert in force at that unit's load-out: the newest cert dated on/before the unit's Load Out List date (explicit load-out/ship date column when the list has one, else the row's Created time; 3-day grace; always capped at today, since some files are named with their calibration DUE date a year out). Old jobs keep the historically-correct cert rather than later recalibrations. Cert dates parse from the MM-DD-YY filename date (SharePoint created date as fallback). Certs outside that rule previously synced into the project are deleted on the next sync. Dedupes by SharePoint item ID. Supports `certsOnly: true` body param to skip subfolders + equipment. |
...
| `upload-po-to-sharepoint` | Admin/customer PO upload via portal + `ingest-po-from-email` | Mirrors a portal-uploaded PO into SharePoint's `Purchase Order/` subfolder, routed by job code to the owning entity's Commercial Proposals tree (HWI → Hydro-Wates, Q → Sentinel Instruments; both in the same document library). If no matching project folder exists, emails sales@ with the PO attached. Records the SharePoint item ID for dedup with future syncs. |
| `ingest-po-from-email` | Power Automate (`PO Email Ingest` flow) | Receives forwarded PO / drawing / photo emails. Zip attachments are extracted in-function (one level deep) and each entry processed like a direct attachment. Claude Opus vision classifies each attachment as PO, drawing, photo, or other. POs upload into the portal AND fire the SharePoint mirror; drawings upload with `kind='drawing'` (project Drawings tab) and photos with `kind='photo'` (project Photos tab) — no SharePoint mirror for either. Native CAD files (.dwg/.dxf/.step) are filed as drawings by extension; camera formats Claude can't read (.heic/.tiff/.bmp) as photos by extension. Projects are matched by job code: `HWI-YY-NNN` (Hydro-Wates) or `QYYNNN` e.g. Q26001 (Sentinel Instruments). Bounces with a friendly explanation if nothing recognized or no job code found. |
...
> When a customer emails you a PO, project drawings, or job-site photos, forward it to `automation@hydrowates.com` with the job code anywhere in the subject — `HWI-26-254` for Hydro-Wates projects, `Q26001` for Sentinel Instruments jobs — plus `#PO`, the word "drawing", or the word "photo" (any casing — "Fwd: Drawings for HWI-26-254" or "Site photos HWI-26-254" work as-is). Zipped attachments are fine — the system unpacks them automatically. Within ~60 seconds you'll get a reply confirming what was filed — POs go to the project's documents + SharePoint (Hydro-Wates or Sentinel Commercial Proposals tree, per the job code), drawings go to the project's Drawings tab, photos go to the project's Photos tab. If the system can't recognize the attachments or find the project, the reply tells you what to fix.
