...
| `sync-files-from-sharepoint` | Project page load (auto-sync) + admin button + bulk-sync script | Pulls files from `Purchase Order/` and `Quote/` subfolders + root `Certificate*.pdf` files for a given project. Equipment certificates come via the loadout pipeline: Load Out List (JobNumber) → shipped Assembly SNs → Hydro-Wates Inventory (FolderPath) → the equipment's Asset Records folder — syncing ONLY the newest cert per unit (ranked by the MM-DD-YY date in the filename; SharePoint created date as fallback), since those folders hold the unit's full recalibration history. Older certs previously synced into the project are deleted on the next sync. Dedupes by SharePoint item ID. Supports `certsOnly: true` body param to skip subfolders + equipment. |
